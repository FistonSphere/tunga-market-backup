...
<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
    @forelse ($products as $product)
        <div class="card group cursor-pointer hover:shadow-hover transition-all duration-300 relative">
            ...
            @if ($product->has_3d_model)
                <div class="absolute top-3 right-3 bg-primary text-white px-2 py-1 rounded-full text-caption font-medium z-10">
                    3D Model</div>
            @elseif($product->is_featured)
                <div class="absolute top-3 right-3 bg-error text-white px-2 py-1 rounded-full text-caption font-medium z-10">Hot
                    Deal</div>
            @elseif($product->is_new)
                <div class="absolute top-3 right-3 bg-success text-white px-2 py-1 rounded-full text-caption font-medium z-10">
                    New Arrival</div>
            @elseif($product->is_best_seller)
                <div class="absolute top-3 right-3 bg-warning text-white px-2 py-1 rounded-full text-caption font-medium z-10">
                    Best Seller</div>
            @elseif($product->stock_quantity <= 5)
                <div class="absolute top-3 right-3 bg-accent text-white px-2 py-1 rounded-full text-caption font-medium z-10">
                    Limited Stock</div>
            @endif

            {{-- === IMAGE DISPLAY === --}}
            <div class="relative overflow-hidden rounded-lg mb-4">
                <img src="{{ $product->main_image ?? asset('assets/images/no-image.png') }}" alt="{{ $product->name }}"
                    ...
                    <div>
                        @if ($product->discount_price)
                            <span class="line-through text-secondary-500 text-sm mr-2">
                                {{ $product->currency === '$' ? $product->currency . number_format($product->price, 2) : number_format($product->price) . ' ' . $product->currency }}
                            </span>
                            <span class="text-subheading font-bold text-primary">
                                {{ $product->currency === '$' ? $product->currency . number_format($product->discount_price, 2) : number_format($product->discount_price) . ' ' . $product->currency }}
                            </span>
                        @else
                            <span class="text-subheading font-bold text-primary">
                                {{ $product->currency === '$' ? $product->currency . number_format($product->price, 2) : number_format($product->price) . ' ' . $product->currency }}
    ...
    @empty
        <p class="text-body-sm text-secondary-600">No products found in this category.</p>
    @endforelse
</div>
