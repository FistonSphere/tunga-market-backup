...
<body class="bg-background text-text-primary">
    ...
    <script>
        (function() {
            ...
            const pager = document.getElementById('rv-pagination');
            ...
            async function initRecentlyViewed() {
                ...
                try {
                    state.ids = JSON.parse(raw) || [];
                } catch (_) {
                    state.ids = [];
                }
                if (!state.ids.length) return;

                const res = await fetch("{{ route('products.recentlyViewed') }}", {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
            ...
            function render() {
                ...
                const items = state.products.slice(start, start + PER_PAGE);

                grid.innerHTML = items.map(cardHtml).join('');
                renderPagination(total, PER_PAGE, state.page);
            }
            ...
            function renderPagination(total, perPage, current) {
                ...
                let html = '';
                html += `<button class="px-3 py-1 rounded border ${current===1?'opacity-50 cursor-not-allowed':''}" 
                        ${current===1?'disabled':''}
                        data-rv-action="page" data-rv-page="${current-1}">Prev</button>`;
                for (let p = 1; p <= totalPages; p++) {
                    html += `<button class="px-3 py-1 rounded border ${p===current?'bg-primary text-white':''}" 
                            data-rv-action="page" data-rv-page="${p}">${p}</button>`;
                }
                html += `<button class="px-3 py-1 rounded border ${current===totalPages?'opacity-50 cursor-not-allowed':''}" 
                        ${current===totalPages?'disabled':''}
                        data-rv-action="page" data-rv-page="${current+1}">Next</button>`;
                pager.innerHTML = html;
            }

            pager?.addEventListener('click', (e) => {
                const btn = e.target.closest('[data-rv-action="page"]');
                if (!btn) return;
                const page = parseInt(btn.dataset.rvPage, 10);
                if (isNaN(page) || page < 1) return;
                const totalPages = Math.ceil(state.products.length / PER_PAGE);
            ...
            grid.addEventListener('click', (e) => {
                ...
                if (!btn) return;
                // Your existing quickAddToCart(btn) will read data-* attributes we set below:
            ...
            function cardHtml(p) {
                // choose display price (discount first)
                const uiPrice = (p.discount ?? null) !== null ? p.discount : p.price;
                ...
                <span class="text-xl font-bold text-primary">
                    ${formatPrice(p.currency, p.discount)}
                </span>
              ` :
                    `<span class="text-xl font-bold text-primary">${formatPrice(p.currency, p.price)}</span>`;
        ...
        <div class="card group cursor-pointer hover:shadow-hover transition-all duration-300">
            <div class="relative overflow-hidden rounded-lg mb-4">
                ...
                <button class="quick-add-cart absolute top-3 left-3 z-20 bg-white/90 backdrop-blur-sm rounded-full p-2 hover:bg-white transition-fast opacity-0 group-hover:opacity-100"
                        title="Quick Add to Cart"
                        data-action="quick-add"
                        data-product-id="${p.id}"
                        data-min-qty="${p.minQty || 1}"
                        data-name="${escapeAttr(p.name)}"
                        data-currency="${escapeAttr(p.currency || '$')}"
                        data-price="${uiPrice}">
                    <svg class="w-5 h-5 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 7M7 13l2.5-7m0 0h9.5M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 7M7 13l2.5-7"></path>
                    </svg>
                </button>
            </div>

            <a href="${p.url}" class="space-y-3">
            ...
            function formatPrice(currency, n) {
                const num = Number(n || 0);
                if (currency === 'Rwf') return `${num.toLocaleString()} ${currency}`;
