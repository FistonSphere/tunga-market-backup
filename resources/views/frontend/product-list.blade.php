...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Build URLSearchParams INCLUDING categories only when selected
  function buildParams() {
    ...
    if (!Number.isNaN(minVal)) params.set('min_price', minVal);
    if (!Number.isNaN(maxVal)) params.set('max_price', maxVal);

    const selectedCategories = Array.from(document.querySelectorAll('.category-checkbox:checked'))
                                    .map(cb => cb.value)
                                    .filter(v => v !== '' && v !== null && v !== undefined);

    // only add categories param if there are any selected
  ...
  // Fetch filtered products and update DOM
  ...
  // Update progress bar (left + width) based on allowed min/max from inputs' attributes
  function updateProgress() {
    const allowedMin = parseFloat(minInput.getAttribute('data-allowed-min')) || parseFloat(minInput.min) || 0;
    const allowedMax = parseFloat(maxInput.getAttribute('data-allowed-max')) || parseFloat(maxInput.max) || 0;
    const curMin = parseFloat(minInput.value) || allowedMin;
    const curMax = parseFloat(maxInput.value) || allowedMax;

    const range = allowedMax - allowedMin;
    if (range <= 0) {
    ...
    const leftPct = Math.max(0, Math.min(100, ((curMin - allowedMin) / range) * 100));
    const widthPct = Math.max(0, Math.min(100 - leftPct, ((curMax - curMin) / range) * 100));
    progressBar.style.left = leftPct + '%';
    progressBar.style.width = widthPct + '%';
  }

  // Load min/max from server for the selected currency and initialize inputs
  async function loadPriceRange() {
    try {
      const currency = currencySelect.value || 'USD';
      const res = await fetch(`/products/price-range?currency=${encodeURIComponent(currency)}`);
      if (!res.ok) throw new Error('Failed to load price range');
      const data = await res.json();

      // Set input attributes so updateProgress can use them
      const allowedMin = parseFloat(data.min) || 0;
      const allowedMax = parseFloat(data.max) || 0;

      minInput.min = allowedMin;
      minInput.setAttribute('data-allowed-min', allowedMin);
      maxInput.max = allowedMax;
      maxInput.setAttribute('data-allowed-max', allowedMax);

      // If user hasn't set values, default them to full range
      if (!minInput.value || isNaN(parseFloat(minInput.value))) minInput.value = allowedMin;
      if (!maxInput.value || isNaN(parseFloat(maxInput.value))) maxInput.value = allowedMax;

      updateProgress();
      debouncedFilter(); // initial fetch with this currency + range
    } catch (err) {
      console.error('loadPriceRange error:', err);
    }
  }

  // Debounced filter for inputs
  const debouncedFilter = debounce(filterProducts, 350);

  // Event bindings
  // - currency change reloads allowed range and filters
  currencySelect?.addEventListener('change', loadPriceRange);

  // - category checkboxes: listen globally (they exist dynamically)
  document.addEventListener('change', (e) => {
    if (e.target && e.target.classList && e.target.classList.contains('category-checkbox')) {
      // immediate filter (not debounced)
      filterProducts();
    }
  });

  // - price inputs: update progress & debounce filter
  minInput?.addEventListener('input', () => {
    // Ensure min <= max
    const minVal = parseFloat(minInput.value);
    const maxVal = parseFloat(maxInput.value);
    if (!isNaN(minVal) && !isNaN(maxVal) && minVal > maxVal) {
      // optional: clamp or swap; here we clamp max to min
