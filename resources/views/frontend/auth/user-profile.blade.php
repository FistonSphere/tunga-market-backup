...
@section('content')
                        ...
                        <div class="card">
                            ...
                            <div class="space-y-8">
                                ...
                                <div class="border-t border-secondary-200 pt-8">
                                    <h3 class="text-lg font-semibold text-primary mb-4">Two-Factor Authentication (2FA)</h3>
                                    ...
                                    @if (auth()->user()->two_factor_enabled)
                                        ...
                                        <button id="disable2faBtn" class="btn-secondary" onclick="disableTwoFactorAuth()">
                                            Disable 2FA
                                        </button>

                                    @else
                                        <!-- 2FA DISABLED STATE -->
                                        <div class="bg-secondary-100 border border-secondary-300 rounded-lg p-4 mb-4">
                                            <p class="text-secondary-700">
                                                Protect your account with an extra security layer.
                                                Enable 2-Step Verification using Google Authenticator or any TOTP app.
                                            </p>
                                        </div>

                                        <button id="enable2faBtn" class="btn-primary" onclick="open2FAModal()">
                                            Enable 2FA
                                        </button>
                                    @endif
                                </div>
                                    ...
                                    <form id="deleteAccountForm" method="POST" action="{{ route('profile.delete') }}">
                                        ...
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <button type="button" class="btn-secondary"
                                                onclick="showDeleteWarningModal()">Delete Account</button>
                                        </div>
                                    </form>
                                </div>
    ...
    <div id="delete-warning-modal-wrapper"
        ...
        <div
            ...
            <!-- Modal Message -->
            <h2 class="text-xl font-bold text-primary mb-3 text-center">Are you sure you want to delete your account?</h2>
            <p class="text-body text-secondary-600 mb-6 leading-relaxed text-center">
    ...
    <!-- 2FA Modal -->
    <div id="twoFaModal" class="fixed inset-0 bg-black bg-opacity-60 hidden flex justify-center items-center" style="z-index:99999999">
    ...
    <script>
        ...
        function showDeleteWarningModal() {
            document.getElementById('delete-warning-modal-wrapper').classList.remove('hidden');
        }

        function closeDeleteWarningModal() {
            document.getElementById('delete-warning-modal-wrapper').classList.add('hidden');
        }
            ...
            fetch(form.action, {
                ...
                .then(async response => {
                    ...
                    if (response.ok) {
                        ...
                        showNotification(data?.message || "❌ Unable to delete account.");
                    }
                })
                .catch(() => {
    ...
    <script>
        function open2FAModal() {
            ...
            document.getElementById('2fa-loading').classList.remove('hidden');

            fetch("{{ route('profile.2fa.generate') }}")
                .then(res => res.json())
                .then(data => {
                    document.getElementById('2fa-loading').classList.add('hidden');
                    document.getElementById('2fa-content').classList.remove('hidden');
        ...
        function verifyTwoFactorCode() {
            ...
            const password = document.getElementById('verify2faPassword').value;

            fetch("{{ route('profile.2fa.enable') }}", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": "{{ csrf_token() }}"
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        alert(data.message || "Invalid authentication code.");
                    }
                });
        }
        ...
        function disableTwoFactorAuth() {
            ...
            const code = prompt("Enter your authenticator code (or recovery code):");

            fetch("{{ route('profile.2fa.disable') }}", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": "{{ csrf_token() }}"
