    ...
    <script>
        ...
        function moveSelectedToWishlist() {
            ...
            if (selectedItems.length === 0) {
                showToast(
                    "No Items Selected",
                    "Please select items to move to wishlist.",
                    "warning"
                );
                return;
            }
            ...
            showToast(
                "Moved to Wishlist",
                `${selectedItems.length} item(s) moved to your wishlist.`
            );
        ...
        function removeSelectedItems() {
            ...
            if (
                ...
                showToast(
                    "Items Removed",
                    `${selectedItems.length} item(s) removed from cart.`
                );
                updateCartItemCount();
            }
        }
        ...
        function requestGroupQuote() {
            ...
            if (selectedItems.length === 0) {
                showToast(
                    "No Items Selected",
                    "Please select items for group quote.",
                    "warning"
                );
                return;
            }
            ...
            showToast(
                "Quote Requested",
                "Your group quote request has been sent to suppliers."
            );
        }
        ...
        function applyCoupon() {
            ...
            if (validCoupons.includes(couponCode)) {
                showToast(
                    "Coupon Applied",
                    `Coupon ${couponCode} has been applied to your order.`
                );
            } else {
                showToast(
                    "Invalid Coupon",
        ...
        function saveForLater(button) {
            ...
            showToast(
                "Saved for Later",
                `${productName} has been saved for later.`
            );
            updateCartItemCount();
        }
        ...
        function calculateShipping() {
            showToast(
                "Shipping Calculated",
                "Shipping options have been updated based on your location."
            );
        }
        ...
        function handleMobileView() {
            const mobileCartSummary = document.getElementById(
                "mobile-cart-summary"
            );
